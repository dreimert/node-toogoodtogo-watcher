...
function createHtmlMessage(businesses) {
  return businesses
    .map(
      (business) =>
      `${business.display_name} : ${business.items_available} à ${business.item.price_including_taxes.minor_units/100} € (${formatInterval(business)}) => https://share.toogoodtogo.com/item/${business.item.item_id}`
    )
    .join("\n\n");
}
